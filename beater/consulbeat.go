				...
				for _, check := range health_service_check {
					event := beat.Event{
						...
						Fields: common.MapStr{
							...
							"AgentAddress":   service_check.Address,
							"CheckID":        check.CheckID,
							"CheckName":      check.Name,
							"Status":         check.Status,
							"Output":         check.Output,
							"ServiceName":    check.ServiceName,
							"ServiceTags":    strings.Join(check.ServiceTags, ","),
							"ServiceID":      service_check.ServiceID,
							"ServicePort":    service_check.ServicePort,
							"ServiceAddress": service_check.ServiceAddress,
						},
					}
